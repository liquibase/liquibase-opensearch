...
import liquibase.ext.opensearch.executor.OpenSearchExecutor;
...
public abstract class AbstractNoSqlHistoryService<D extends AbstractNoSqlDatabase> extends AbstractChangeLogHistoryService {
    ...
    public OpenSearchExecutor getExecutor() throws DatabaseException {
        Executor executor = Scope.getCurrentScope().getSingleton(ExecutorService.class).getExecutor(OpenSearchExecutor.EXECUTOR_NAME, getDatabase());
        if (executor instanceof LoggingExecutor) {
            throw new DatabaseException(String.format(resourceBundle.getString("command.unsupported"), "*sql"));
        }
        return (OpenSearchExecutor) executor;
    }
    ...
    public void reset() {
        ...
        // out sequence values based on a stale read of the changelog index.
    ...
    protected abstract int generateNextSequence() throws DatabaseException;
